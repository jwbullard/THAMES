...
<b>4. Results Tab:</b> View and analyze your results</span>
    ...
    def _on_troubleshooting_clicked(self, menu_item):
        ...
        doc_viewer = get_documentation_viewer()
        # Route to Section 11 of the User Manual ("Troubleshooting").
        # The anchor "11-troubleshooting" matches the id generated by
        # Python-Markdown's toc extension on the "## 11. Troubleshooting" heading.
        doc_viewer.open_section("11-troubleshooting", self)
    ...
    def _show_about_dialog(self):
        ...
        # Set basic program info. Pull version from app_info so the string is
        # maintained in a single place.
        from app.resources import app_info as _app_info
        about_dialog.set_program_name("THAMES")
        about_dialog.set_version(_app_info.APP_VERSION)
        # Make the alpha / beta / rc stage visible next to the tagline so
        # users immediately recognize a pre-release build.
        stage = getattr(_app_info, "APP_VERSION_STAGE", "release")
        tagline = _app_info.APP_TITLE
        if stage != "release":
            tagline = f"{tagline}  —  {stage.upper()} pre-release, not for production use"
        about_dialog.set_comments(tagline)
        about_dialog.set_website(_app_info.APP_WEBSITE)
        about_dialog.set_website_label("THAMES GitHub Repository")

        # Set authors. The Credits sub-dialog renders each line through Pango
        # markup, so any '&' must be pre-escaped as '&amp;' — otherwise Pango
        # tries to parse it as an HTML entity (e.g. &amp;, &lt;), fails, and
        # falls back to showing the raw source wrapped in <span> tags. See:
        # https://docs.gtk.org/Pango/pango_markup.html
        def _escape_pango(s: str) -> str:
            return s.replace('&', '&amp;')
        about_dialog.set_authors([_escape_pango(a) for a in _app_info.AUTHORS])

        # Copyright — rendered as plain text (no markup), so no escape needed.
        about_dialog.set_copyright(f"© 2024 {_app_info.ORG_NAME}")
        
        # License text is the canonical copy from app_info (single source of truth).
        about_dialog.set_license(_app_info.LICENSE_TEXT)
