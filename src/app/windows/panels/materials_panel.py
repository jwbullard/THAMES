...
from app.windows.dialogs.thames_material_dialog import MaterialDialog
...
class MaterialsPanel(Gtk.Box):
    ...
    def __init__(self, main_window: 'VCCTLMainWindow'):
        ...
        try:
            ...
            self.logger.info("THAMES MaterialService initialized successfully")
        except Exception as e:
            self.logger.warning(f"Could not initialize THAMES MaterialService: {e}")
    ...
    def _on_add_material_clicked(self, button) -> None:
        """Handle add material button click - use THAMES MaterialDialog."""
        try:
            # Check if THAMES MaterialService is available
            if not self.material_service:
                self.logger.warning("THAMES MaterialService not available, using VCCTL dialogs")
                # Fallback to old VCCTL style
                self._show_vcctl_add_material_dialog()
                return

            # Use new THAMES MaterialDialog
            ...
            self.logger.error(f"Error opening material dialog: {e}")
            self.main_window.update_status(f"Error: {str(e)}", "error", 5)

    def _show_vcctl_add_material_dialog(self) -> None:
        """Fallback: Show old VCCTL material type selection dialog."""
        dialog = Gtk.Dialog(
            title="Add Material",
    ...
    def _on_material_activated(self, table, material_data) -> None:
        ...
        if material_type == 'thames':
            ...
            # Use old VCCTL dialogs for legacy materials
    ...
    def _on_delete_material_clicked(self, button) -> None:
        ...
        # Check if THAMES material is immutable before showing confirmation
        material_type = self._get_material_type(self.selected_material)
        if material_type == 'thames':
            if hasattr(self.selected_material, 'immutable') and self.selected_material.immutable:
                # Show error dialog instead of confirmation
                ...
                self.main_window.update_status(
                    f"Cannot delete '{self.selected_material.name}': Material is read-only",
                    "error", 5
                )
                return
    ...
    def _copy_thames_material_data(self, original_material, new_name: str) -> dict:
        """Create a copy of THAMES material data."""
        copy_data = {
            'name': new_name,
            'tags': original_material.tag_names if hasattr(original_material, 'tag_names') else [],
            'specific_gravity': original_material.specific_gravity,
            'specific_surface_area': original_material.specific_surface_area,
            'psd_data_id': original_material.psd_data_id,
