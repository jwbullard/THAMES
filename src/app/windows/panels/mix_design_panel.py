...
Based on VCCTL v10.0.0 with THAMES-specific extensions for dynamic phase ID mapping.
"""
...
class MixDesignPanel(Gtk.Box):
    ...
    def __init__(self, main_window: 'VCCTLMainWindow'):
        ...
        self.logger = logging.getLogger('THAMES.MixDesignPanel')
    ...
    def _create_content_area(self) -> None:
        ...
        left_frame = Gtk.Frame(label="Binder Components")
    ...
    def _create_component_row(self) -> Dict[str, Any]:
        ...
        THAMES simplified version: Uses MaterialSelector with autocomplete and tag filtering.
        """
        ...
        # THAMES: Material selector with autocomplete and optional tag filter
        material_service = self.service_container.material_service
        material_selector = MaterialSelector(
            material_service,
            show_tag_filter=True,
            placeholder="Type to search materials..."
        )
        row_box.pack_start(material_selector, False, False, 0)
        ...
        # Grading button (hidden for binder components)
        ...
        grading_button.set_sensitive(False)
        grading_button.set_no_show_all(True)  # Hide for binder components
        grading_button.hide()
        ...
        # Connect signals
        material_selector.connect('material-selected', self._on_material_selector_changed, row_data)
        material_selector.connect('material-cleared', self._on_material_selector_cleared, row_data)
    ...
    def _on_material_selector_changed(self, selector, material_id: int, material_name: str, sg: float, row_data: Dict[str, Any]) -> None:
        """Handle material selection from MaterialSelector widget."""
        row_data['material_id'] = material_id
        self.logger.debug(f"Material selected: {material_name} (ID: {material_id}, SG: {sg})")

        # Trigger recalculation
        if self.auto_calculate_enabled:
            self._trigger_calculation()

    def _on_material_selector_cleared(self, selector, row_data: Dict[str, Any]) -> None:
        """Handle material cleared from MaterialSelector widget."""
        row_data['material_id'] = None

        # Trigger recalculation
        if self.auto_calculate_enabled:
            self._trigger_calculation()

    # THAMES-specific methods for simplified material selection
    def _populate_thames_material_combo(self, combo: Gtk.ComboBoxText) -> None:
        """Populate combo with all THAMES materials.

        THAMES uses a unified material system with tags instead of rigid categories.
        All materials are shown in a single alphabetically-sorted dropdown.
        """
        combo.remove_all()
        combo.append("", "-- Select Material --")

        try:
            # Get MaterialService from service container
            material_service = self.service_container.material_service

            # Get all THAMES materials
            materials = material_service.get_all()

            # Sort by name and add to combo
            for material in sorted(materials, key=lambda m: m.name.lower()):
                # Store material ID as the combo ID for easy lookup
                combo.append(str(material.id), material.name)

            self.logger.debug(f"Populated material combo with {len(materials)} materials")

        except Exception as e:
            self.logger.error(f"Failed to populate THAMES material combo: {e}")

    def _on_thames_material_changed(self, combo: Gtk.ComboBoxText, row_data: Dict[str, Any]) -> None:
        """Handle THAMES material selection change.

        Updates the specific gravity display and triggers recalculation.
        """
        material_id_str = combo.get_active_id()
        ...
        try:
            ...
            # Get material from service
            material_service = self.service_container.material_service
            material = material_service.get_by_id(material_id)

            if material and material.specific_gravity:
                sg_label.set_text(f"SG: {material.specific_gravity:.3f}")
            else:
                sg_label.set_text("SG: --")
