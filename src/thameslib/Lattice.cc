...
Lattice::Lattice(ChemicalSystem *cs, RanGen *rg, int seedRNG,
  ...
  // cout << endl << "Lattice::Lattice   latticeRNGseed_ = " <<
  // latticeRNGseed_
  ...
  //   exit(1);
  // }
  // //cout << endl << "Lattice::Lattice   rg_->getSeed() = " <<
  // rg_->getSeed()
  // << endl;
  numRNGcall_0_ = 0;
...
void Lattice::nucleatePhaseRnd(const int phaseID, const int numToNucleate) {
  ...
  ///
  /// @todo JWB Prefer saturated capillary pore voxels, but then allow
...
vector<int> Lattice::dissolvePhase(vector<int> dissPhaseIDVect,
      ...
      if (numLeft[i] == 0) {
        if (writeFirst[i]) {
          ...
          cout << "    Lattice::dissolvePhase DISS_END for i = " << i
               ...
               << bcl << endl;
          cout << "      DISS_END dissPhaseIDVectSize = " <<
          dissPhaseIDVectSize
               << "   dissolutionVectorSize = " << dissolutionVectorSize
               << "   numLeftTot = " << numLeftTot
               << "   numChangeTot = " << numChangeTot << endl;
          cout << "        DISS_END phaseid count_ dim_isite numleft
          numchange "
                  ":  "
  ...
  site_[i].getInDissolutionVectorPos() << endl; cout << "     *** out
  ...
  */
  ...
  trc_d/countErrorVect/countErrorInt = " << trc_d
          << " / " << countErrorVect << " / " << countErrorInt << endl <<
  endl;
...
void Lattice::addGrowthSite(Site *ste, int pid) {
  ...
  // } catch (out_of_range &oor) {
  //   cout << endl << "EOB Lattice::addGrowthSite pid = " << pid << "
  //   => exit"
  //   << endl; exit(1);
  // }
}
...
void Lattice::removeGrowthSite_nucleation(Site *ste0) {
  ...
  ste0->clearGrowth();
  // } catch (out_of_range &oor) {
  //   cout << endl << "EOB Lattice::removeGrowthSite_nucleation pid =
  //   " << pid
  //   << " => exit" << endl; exit(1);
  // }
}
...
void Lattice::removeGrowthSite_grow(Site *ste0, int grPhId) {
  ...
  growthInterfaceSize_[grPhId]--;
  // } catch (out_of_range &oor) {
  //   cout << endl << "EOB Lattice::removeGrowthSite_grow grPhId = "
  //   << grPhId
  //   << " => exit" << endl; exit(1);
  // }
}
...
void Lattice::removeGrowthSite_diss(Site *ste0, int pid) {
  ...
  growthInterfaceSize_[pid]--;
  // } catch (out_of_range &oor) {
  //   cout << endl << "EOB Lattice::removeGrowthSite_diss pid = " <<
  //   pid << "
  //   => exit" << endl; exit(1);
  // }
}
...
double Lattice::emptySubVoxelPorosity(double volFracToRemove, const int cyc) {
  ...
  for (int i = masterPoreVolumeSize - 1; (i >= 0) && (volFracToRemove > 0.0);
       --i) {

    // volfrac_filled is the volume fraction of size-i saturated porosity on
    // a total microstructure volume basis
    volfrac_filled = masterPoreVolume_[i].volume * masterPoreVolume_[i].volfrac;
    ...
    if (newvfracfilled < 0.0)
      newvfracfilled = 0.0;
    masterPoreVolume_[i].volfrac = newvfracfilled / masterPoreVolume_[i].volume;
...
double Lattice::fillSubVoxelPorosity(double volFracToAdd, const int cyc) {
  ...
  int masterPoreVolumeSize = masterPoreVolume_.size();
  for (int i = masterPoreVolumeSize - 1; (i >= 0) && (volFracToAdd > 0.0);
       --i) {

    // volfrac_empty is the volume fraction of size-i empty porosity on
    // a total microstructure volume basis
    volfrac_empty =
        masterPoreVolume_[i].volume * (1.0 - masterPoreVolume_[i].volfrac);
    ...
    if (newvfracempty < 0.0)
      newvfracempty = 0.0;
    masterPoreVolume_[i].volfrac =
        1.0 - (newvfracempty / masterPoreVolume_[i].volume);
...
int Lattice::emptyCapillaryPorosity(int numToEmpty, const int cyc) {
  int maxsearchsize = 3;
  Site *stenb;
...
int Lattice::fillCapillaryPorosity(int numToFill, const int cyc) {
...
int Lattice::changeMicrostructure(double time, const int simtype,
  ...
  // checkSite(8); cout << endl << " exit
  // Lattice::changeMicrostructure " << endl;
  ...
  endl; cout << "   waterDCMolesChemSys = " << waterDCMolesChemSys <<
  endl; cout << "   waterDCMolesLattice = " << waterTotMoles <<
  endl; cout << "   waterTotMoles_0 = " << waterTotMoles_0 <<
  endl; cout << "   initialMicrostructureVolume_ = "<<
  ...
  if (simtype == SULFATE_ATTACK && time_ >= sulfateAttackTime_) {
    ...
    */
    try {
      ...
      // vector<int> transform(int alphaseid, int netsitesAlphaseid, int
      ettrid,
  ...
  if (verbose_) {
    for (i = 0; i < volSize; ++i) {
      cout << "Lattice::adjustMicrostructureVolFracs "
           << "volume fraction of " << names[i] << " should be " << vfrac[i]
           << ", (" << vol[i] << "/"
...
void Lattice::writePoreSizeDistribution(const double curtime,
  ...
  // microstructureVolume_ << endl; cout << "--> capillaryPoreVolume_
  // : " << capillaryPoreVolume_ << endl; cout << "-->
    ...
    try {
      ...
      if (ste->getInDissInterfacePos() == -1) {
        cout << endl
             ...
             << endl;
        cout << "    Lattice::transformPhase error: steId/pid/posVect  "
             << ste->getId() << "/" << pid << "/" << posVect << endl;
      ...
      if ((waterneighborSize + 1) <= max) { // count the site itself
        ...
        // cout << "Lattice::transformPhase fileName = " << fileName <<
        // endl;
        ...
        if (exp > 0) {
          ...
          //        << " / " << exp << " / " << ste->getId() << " / " << pid <<
          //        endl;
          //   cout << endl << "    Lattice::transformPhase exit" <<
          //   endl; exit(0);
        }
