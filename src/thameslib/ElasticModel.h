/**
@file ElasticModel.h
@brief Declaration of the ElasticModel base class for finite element
calculations.
...
protected:
  ...
  */
  ...
  The stiffness matrix relates the local displacement field of an element
  (defined at its eight corners) to the local stress tensor components, and is
  the centeral material property used to compute the contributions to the
  quadratic term of the energy function.  The indices have the following meaning

      - First index is the phase index number
      - Second index can be 0 to 8, defining which corner of the cubic element
      - Third index can be 0 to 3, defining the components of the displacement
  at that corner
  ...
  */
  ...
  std::vector<int> pix_;                       /**< Phase index label at each
                                                    element */
  std::vector<std::vector<double>> elestress_; /**< Stress components at each
                                                    element */
  std::vector<std::vector<double>> elestrain_; /**< Strain components at each
                                                    element */
  std::vector<double> strainengy_;             /**< Local strain energy in each
                                                    element */
  ...
  This the maximum allowed square of the energy gradient for which the whole
  system can be considered to have converged to the energy minimum.  This is the
  value to which the quantity `gg_` (the computed square of the energy gradient,
  `gb_`)is compared. In most cases, `gtest_` will be set equal to a value
  <i>abc</i> times the total number of elements in the mesh, so that when `gg_`
  is less than `gtest_`, the RMS gradient per element is less than
  \f$\sqrt{abc}\f$.
  */
  double gtest_;
...
public:
  ...
  */
  ...
  @param i is 0 (Young's or bulk modulus) or 1 (Poisson's ratio or shear
  modulus)
  ...
  */
  ...
  @remarks There is a variable used in this function, called `strainenergy`,
  which is defined in the StrainEnergy.h header file, and it is unclear what
  this variable does or why it is set the way it is.  It does not appear to be
  ...
  */
  double getElestress(int m, int index) { return elestress_[m][index]; }
  ...
  */
  ...
  @todo Use the libpng library to avoid having to rely on system call to
  ImageMagick
