...
**Next Session TODO**:
1. Try building with Clang on Windows instead of GCC:
   ```bash
   pacman -S mingw-w64-x86_64-clang
   cmake -G "MinGW Makefiles" -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ ..
   ```
2. Compare input files generated by Mac vs Windows UI for test operation
3. Debug Windows crash if Clang build doesn't resolve it
